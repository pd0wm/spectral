...
Given the complex gaussian random variable $Z = X + jY$, then $Z$ is said to be complex gaussian distributed if $X$ and $Y$ are jointly-gaussian distributed. We denot $Z \sim \mathcal{CN}{\mu, \Gamma, C}$.
\end{blockDefinition}
...
\end{align*}
it follows that $Z$ has an expectation value of zero.% ($E(Z) = 0$) and that %$E(Z^2)= E(X^2 - Y^2 + 2jXY] = 0$.  
...
\end{align*}
where $\eta$, the decision threshold, is chosen such that $P[\Lambda(\vec{x} < \eta) | \mathcal{H}_1]$ is minimized subject to $P[(]\Lambda(\vec{x}) > \eta) | \mathcal{H}_0] = P\ss{fa}$, where $P\ss{fa}$ denotes the false alarm probability. % insert ref naar boek
\end{blockDefinition}
