...
Note that $\odot$ is similar to $\cdot$, since the inner product and Hadamard product are closely related. The following theorem identifies the expected value of the correlation operator.
...
\begin{blockDefinition} \lab{def:dots-extended}
    Let $N \in \mathbb{N}$ and let $S=\{(a,b) \in \mathbb{N} \times \mathbb{N} : a \le b \le N\}$. Then $(1,1),\ldots,(N,N)$ denotes the sequence of all elements in $S$ such that $(a,b) \in S$ comes before $(c,d) \in S$ when $a < c$, or when $a = c$ and $b < d$. 
\end{blockDefinition}

\begin{blockTheorem} \lab{th:dots-extended}
    Let $N \in \mathbb{N}$. Then $(1,1),\ldots,(N,N)$ is finite and has length $\frac{1}{2}N(N+1)$.
\end{blockTheorem}
...
\begin{blockProofTheorem}{\ref{th:dots-extended}}
    ...
    \begin{align*}
        ...
        \underbrace{(2,1),(2,2)}_{\text{length } 2},
        \underbrace{(3,1),\ldots,(3,3)}_{\text{length } 3},
        \ldots,
        \underbrace{(N,1),\ldots,(N,N)}_{\text{length } N}.
    \end{align*}
    So $(1,1),\ldots,(N,N)$ has length
    \begin{align*}
