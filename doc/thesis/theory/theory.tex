...
\begin{blockDefinition}[Window Vector]
...
Finally, let $N \in \mathbb{N}$. Then $(1,1),\ldots,(N,N)$ denotes
\begin{align*}
    (1,1),\ldots,(1,N),(2,1),\ldots,(2,N),\ldots,(N,N).
\end{align*}

\section{Overview of Variables}
\Cref{tab:overview-vars-main-analysis} presents an overview of the variables which will be used in the main analysis.
...
\begin{table}[H]
    ...
    \begin{tabular}{p{5cm}ll}
        ...
        Correlation of input & $\vec{r}_x$ & $2LN - 1$ \\
        Correlation of pseudo outputs of cosets $i$ and $j$ & $\vec{r}_{y_i,y_j}$  & $2(2 LN + N - 1) - 1$ \\
        Correlation of sampling vectors of cosets $i$ and $j$ & $\vec{r}_{c_i,c_j}$  & $2N-1$ \\
        Truncated correlation of pseudo outputs of cosets $i$ and $j$ & $\hat{\vec{r}}_{y_i,y_j}$  & $2LN - 1$ \\
        Convolution matrix of cosets $i$ and $j$ & $\mat{R}_{c_i,c_j}$ & $2LN-1 \times 2LN-1$ \\
        Decimation matrix & $\mat{D}$ & $2L-1 \times 2LN - 1$ \\
        Stacked convolution matrices of cosets & $\vec{R}$ & $M^2 (2LN - 1) \times (2LN - 1)$ \\
        Correlation of outputs of cosets $i$ and $j$ & $\vec{r}_{y'_i,y'_j}$ & $2L-1$ \\
        Stacked correlation of outputs of cosets & $\vec{r}'_y$ & $M^2(2L-1)$ \\
        Interval $m$ of the input signal & $\vec{x}_m$ & $N$ \\
        Sampling vector of coset $i$ reversed & $\vec{d}_i$ & $N$ \\
        Output of all costest in interval $m$ & $\vec{w}_m$ & $M^2$
    \end{tabular}
...
\end{align*}
Thus $\vec{r}_{y'_i,y'_j}$ is the $N$-decimation of $\vec{r}_{y_i,y_j}$. Let $\vec{y}'_i$ denote the $N$-decimation of $\vec{y}_i$. Then $\vec{y}'_i$ corresponds to the output of coset $i$.
