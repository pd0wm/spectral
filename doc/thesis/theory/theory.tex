...
\begin{blockDefinition} \lab{def:dots-extended}
    Let $N \in \mathbb{N}$ and let $S=\{(a,b) \in \mathbb{N} \times \mathbb{N} : a < b \le N\}\cup \{(1,1)\}$. Then $(1,1),\ldots,(N,N)$ denotes the sequence of all elements in $S$ such that $(1,1)$ is first and $(a,b) \in S$ comes before $(c,d) \in S$ when $a < c$, or when $a = c$ and $b < d$.
\end{blockDefinition}
...
\begin{align*}
    ...
    &= %\hskip \textwidth minus \textwidth \\ % Fill line \intertext{\hfill$ $}
    \begin{bmatrix}
...
\begin{blockProofTheorem}{\ref{th:corr-unbiased}}
    ...
    \begin{align*}
        ...
        &= M R_{X,Y}[i-M]
    \end{align*}
    since $X[n]$ and $Y[n]$ are wide sense stationary. So $E(r[i])=M R_{X,Y}[i]$ for $-M+M<i<N-M$. Finally suppose that $i \ge N$, then
    \begin{align*}
...
\begin{blockProofTheorem}{\ref{th:deci-corr}}
    ...
    Therefore
    \begin{align*}
        [E(N\vec{r}_{y'_i,y'_j})]_m
        &= N W[m-L] R_{Y'_i,Y'_j}[m-L] \\
        &= N W[m-L] R_{Y_i,Y_j}[mN-LN]
        % &= E(\vec{r}'_{y_i,y_j}).
    \end{align*}
    where
    \begin{align*}
        NW[m-L] &= N\begin{cases}
            m & 1 \le i \le L, \\
            2L-m & L \le i \le 2L - 1, \\
            0 & \text{elsewhere}
        \end{cases} \\
        &= \begin{cases}
            mN & N \le iN \le LN, \\
            2LN-mN & LN \le iN \le 2LN - N,\\
            0 & \text{elsewhere}
        \end{cases} \\
        &= \begin{cases}
            mN & 1 \le iN \le LN, \\
            2LN-mN & LN \le iN \le 2LN - 1, \\
            0 & \text{elsewhere}.
        \end{cases}
    \end{align*}
    We now recognise that
    \begin{align*}
        [E(N\vec{r}_{y'_i,y'_j})]_m &= [E(\vec{r}_{y_i,y_j})]_{mN} = [E(\vec{r}'_{y_i,y_j})]_m.
    \end{align*}
\end{blockProofTheorem}
...
Second, $E(\vec{r}_x)$ is usually estimated to estimate the power spectral density of $X[n]$. Therefore, we study the effect of the weights discussed in \cref{th:corr-unbiased} on the power spectral density. By the Wiener-Khintchine theorem and the convolution theorem,
\begin{align*}
