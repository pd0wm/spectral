...
Circular sparse sampling uses samplers with sampling period $NT$, where $T$ is the sampling period of the signal to be reconstructed and $N$ the down-sampling factor. This is illustrated in \cref{tkz:circsparseruler} where $N = 7$.
...
\end{figure}
The grey blocks represent the samples that are recovered by the device and the different coloured blocks represent the samples that are recovered by the different samplers. An implementation of this sampling method is illustrated in \cref{tkz:sc_circsparseruler}. 
...
\end{figure}
In this figure, the top half is device one, and the bottom half is device 2. The grey blocks represent the samples that are recovered by each device, and the coloured blocks represent the samplers that are recovered by each sampler.
...
The grey blocks represent the samples that are recovered by the device, and the coloured blocks represent the samplers that are recovered by each sampler.

The choice of the down-sampling factors substantially influences the performance of the device and will be discussed further in \cref{sec:coprime}. 

It is not trivial to feed a coprime sampler to the reconstructor. The reconstructor expects a set of different devices with the same sampling period. To put the coprime sampler into the reconstructor, we need some additional processing. We use the signals of two samplers with down-sampling factor $n$ and $m$ to make $n+m$ signals with down-sampling factor $n\cdot m$. This will be further explained in \cref{sec:coprime}. A schematic for a possible hardware implementation is given by \cref{tkz:sc_coprime}. 
