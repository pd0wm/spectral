...
\end{enumerate}
We require that columns $1$ to $N$ are nonzero. This implies the following statement. If $|n_i - n_j|$ and $N-|n_i-n_j|$ for all combinations $i$ and $j$ make up $0$ to $N-1$, then all columns of $\mat{R}$ are nonzero and $\mat{R}$ has full column rank. This restriction will be further discussed in \ref{cha:sampling_methods}.
