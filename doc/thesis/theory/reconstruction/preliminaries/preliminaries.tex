...
We also need notation to denote two uncommon vector operations.
...
It is important to notice that the convolution operator and deterministic cross-correlation operator are equivalent to respectively \texttt{conv} and \texttt{xcorr} in MATLAB. Also, the deterministic cross-correlation turns out to be a biased estimator of the cross-correlation. We furthermore define the Hadamard product as a closed binary operation on vector.
...
Note that $\odot$ is similar to $\cdot$, since the inner product and Hadamard product are closely related. Also, the Hadamard product is equivalent to element-wise multiplication in MATLAB.
