    ...
    \begin{tikzpicture}
        \newcommand{\drawitem}[3]{
            \begin{scope}[shift={(0,#1*-3)}]
                ...
                \draw [black, thick, >=latex, ->] (-2.05+#1-1,-1.25) -- node[pos=0.45,fill=white] {\tiny$\sum$} (-3.45,-1.25);
                \draw [black, thick] (-5,-1) -- (-5,-1.5) -- (-3.5,-1.5) -- (-3.5,-1) -- (-5,-1);
                \draw (-4.25,-1.25) node[black] {$(\vec{x} \circ \vec{y})_{#1}$};
            \end{scope}
        }
        \drawitem{1}{0}{0}
        \drawitem{2}{0}{1}
        ...
        \drawitem{6}{3}{3}
    \end{tikzpicture}
    \caption{Visualisation of $\vec{x} \circ \vec{y}$ where $\vec{x} \in \mathbb{C}^4$ and $\vec{y} \in \mathbb{C}^3$}
...
\begin{blockTheorem} \lab{th:correlation-bias}
    ...
    \makebox[\linewidth]{
        $\displaystyle [E(\vec{x} \circ \vec{y})]_{i+N} = (N-|i|)R_{X,Y}[i]$.
    } \nolinebreak
\end{blockTheorem}
...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \draw [black,thick] (-4,0) -- (-4,0.6) -- (0,1.5) -- (4,0.6) -- (4,0);
        \draw [dashed] (-5,1.5) -- (5,1.5);
        \draw (5.5,1.5) node[black] {$N$};
        \draw (5.5,0.6) node[black] {$1$};
        \draw (-4,-0.4) node[black] {$R_{X,Y}[-N+1]$};
        \draw (0,-0.4) node[black] {$R_{X,Y}[0]$};
        \draw (4,-0.4) node[black] {$R_{X,Y}[N-1]$};
    \end{tikzpicture}
