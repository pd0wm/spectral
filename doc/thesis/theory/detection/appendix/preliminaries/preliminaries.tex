...
\begin{enumerate}
	\item $\mu = E(Z)$
	\item $\Gamma = E\left(Z-\mu)(\conj(Z)-\conj(\mu))^T\right)$
...
Given a complex gaussian random variable $Z = X + jY$, then $Z$ is circular symmetric complex gaussian distributed if $Z$ has the same distribution as $Ze^{j\theta}$ with $\theta \in \mathbb{R}$. Since
\begin{align*}
