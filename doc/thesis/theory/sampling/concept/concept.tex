...
When one desires to ensure full reconstruction of the signal with uniform sampling one needs to sample at the Nyquist rate\footnote{Nyquist rate is the minimum rate at which a signal can be sampled without introducing errors, which is twice the highest frequency present in the sitgnal}. However, we don't need a full recovery of the signal, and are only interested in certain properties of the signal. To improve our sampling methods we remove the restriction of uniform sampling to look for more efficient ways to sample the spectrum, and efficiently recover the information we are looking for. Non-uniform sampling has no restriction on delays of subsequent samples as is illustrated in \cref{tkz:nonuniform}.
...
\draw  (v5) node {T} ;
\end{tikzpicture}
\caption{Non-uniform sampling with sampling period $T$. Gray blocks are sampled}\label{tkz:nonuniform}
\end{figure} 
...
Here the $x$ represents the input signal, the switches are the samplers and $y_i$ are the outputs of the different samplers. In the reconstruction section we will deduce criteria on the sampling that will further specify the samplers. Wee will then get back to multi-coset sampling to link it to the reconstruction and optimize performance in \cref{cha:sampling_methods}.
