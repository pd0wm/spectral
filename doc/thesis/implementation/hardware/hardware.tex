...
To verify that our synchronisation is correct, we dumped two frames from the same moment in time to a file. If we plot them on the same time scale, as in figure ***, we can see that they align almost perfectly. We can also calculate the cross-correlation between the two signals, which looks like figure ***.

The only offset we have is from the phase offset between the two local oscillators of the USRPs. After each tune command the two USRPs have a random phase offset of \SIrange[retain-explicit-plus]{-180}{+180}{\degree}.  We don't know if that influences our results. If that is the case we can calculate this lag by looking at the cross-correlation between the two USRPs or by transmitting a reference pulse after each tune command and calculating the delay.


Hier nog plotjes zetten van sync na x frames. Hier maandag even samples van binnen halen.
