...
Unit tests are small tests that test on a function level. They test if each function returns the right values for given input signals and tests for edge cases. These unit tests should be fast and be ran often while writing code. This way, the correctness of one's code may be verified at all times. When the tests pass, the code is guaranteed to work according to the specifications. This becomes particularity important when working on a large project with multiple people. Any change in code can be verified quickly. Writing unit tests also forces the developer to think more about their code and stimulates modular design.

It is also possible to write the tests before the implementation of a function. This is called test driven development (TDD). In our design we decided not to use this, because the specificationse were not entirely clear at the start of the project. In case of an abundance of additional time, TDD would definitely be a preferred approach.
