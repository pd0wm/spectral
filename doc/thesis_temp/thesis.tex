...
\end{align*}
So $\vec{r}_y'$ is an unbiased estimator of $\mat{R} E(\vec{r}_x)$, which we can use to determine or estimate $E(\vec{r}_x)$. Finally, note that
\begin{align*}
    (\vec{y}'_i)_m = (\vec{y}_i)_{mN} = (\vec{c}_i \ast \vec{x})_{mN} = \hat{\vec{c}}_i \cdot \vec{x}[(m-1)N+1,mN]
\end{align*}
where $\hat{\vec{c}}_i$ denotes $\vec{c}_i$ reversed. Therefore
\begin{align*}
    \begin{bmatrix}
        (\vec{y}'_1)_m \\
        \vdots \\
    ...
    \end{bmatrix} = \begin{bmatrix}
        \hat{\vec{c}}_1 \cdot \vec{x}[(m-1)N+1,mN] \\
        \vdots \\
        \hat{\vec{c}}_M \cdot \vec{x}[(m-1)N+1,mN]
    \end{bmatrix} = \begin{bmatrix}
        \hat{\vec{c}}_1^T\\
        \vdots \\
        \hat{\vec{c}}_M^T
    \end{bmatrix} \vec{x}[(m-1)N+1,mN].
\end{align*}
...
\begin{enumerate}
    ...
    \item Determine $y'_i$ for $i = 1,\ldots,M$.
