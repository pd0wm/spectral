...
\end{theorem}
\begin{proof}
    A simple change of index yields that
    \begin{align*}
        (\vec{x} \ast \vec{y})_i &= \sum_{k=1}^{N} (\vec{x})_k (\vec{y})_{i-k+1} \\
        &= \sum_{k=-\infty}^{\infty} (\vec{x})_k (\vec{y})_{i-k+1} \\
        &= \sum_{k'=-\infty}^{\infty} (\vec{x})_{i-k'+1} (\vec{y})_k' \\
        &= \sum_{k'=-\infty}^{\infty} (\vec{y})_k' (\vec{x})_{i-k'+1} \\
        &= \sum_{k'=1}^{M} (\vec{y})_k' (\vec{x})_{i-k'+1} \\
        &= (\vec{y} \ast \vec{x})_i.
    \end{align*}
\end{proof}
...
\begin{align*}
    ...
    &=\sum_{k''=1}^{LN+N-1}\sum_{k=1}^N (\vec{c}_i)_k (\vec{x})_{k''-k+1}\sum_{k'=1}^{N}(\vec{c}_j)_{k'}(\vec{x})_{(LN+N-1-l+k'')-k'+1} \\
